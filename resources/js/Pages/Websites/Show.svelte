<script lang="ts">
  ...
  const eventSnippet = '<button data-palantir-event="pricing-cta">Start free trial</button>'
  const manualEventSnippet = "window.palantir.track('purchase', { plan: 'pro' })"
</script>
...
<DashboardLayout {websites} {website} section="tracking">
  <div class="mx-auto max-w-4xl space-y-6">
    ...
    <Card.Root><Card.Header><Card.Title>Track clicks</Card.Title><Card.Description>Add <code>data-palantir-event</code> to any element. Its value appears as the event name in your dashboard.</Card.Description></Card.Header><Card.Content class="space-y-4"><pre class="overflow-x-auto rounded-md border bg-muted p-4 text-sm"><code>{eventSnippet}</code></pre><div><p class="mb-2 text-sm text-muted-foreground">For events with extra data, use the JavaScript API:</p><pre class="overflow-x-auto rounded-md border bg-muted p-4 text-sm"><code>{manualEventSnippet}</code></pre></div></Card.Content></Card.Root>
